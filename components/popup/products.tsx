...
export default function ProductPopup(props: ProductPopupProps){
  ...
  const getProducts = async () => {
    ...
    const products = await (await fetch(url)).json() as Product[]
    setProducts(products)
  }

  useEffect(() => {
    if (props.open) getProducts()
  }, [props.open]) 
            ...
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    >
                      { products?.map( p => 
                        <SwiperSlide key={p.productId}>
                          <div className="w-full max-w-sm bg-white rounded-lg shadow-md" key={p.productId}>
                            <Link href={`p/${p.productId}/i/${props.itemId}`}>
                              <span>
                                <Image className="p-8 rounded-t-lg" src={p.images[0].preview.url} 
                            ...
                            <div className="px-5 pb-5">
                              <Link href={`p/${p.productId}/i/${props.itemId}`}>
                                <span>
                                  <h5 className="text-xl font-semibold tracking-tight text-gray-900">{p.title}</h5>
                                </span>
                              </Link>
                              <div className="flex items-center mt-2.5 mb-5">
                                ...
                                <span className="bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded ml-3">4.7</span>
                              </div>
                            </div>
                          </div>
                        </SwiperSlide>
                  ...
                  <div className="hidden lg:grid mt-2 grid-flow-col gap-3">
                      { products?.map( p => 
                        <div className="w-full max-w-sm bg-white rounded-lg shadow-md" key={p.productId}>
                          <Link href={`p/${p.productId}/i/${props.itemId}`}>
                            <span>
